...
const int ZERO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ZERO_IPC_PREFIX("zero:");
...
// const char* BIP71_MIMETYPE_PAYMENT = "application/zero-payment";
// const char* BIP71_MIMETYPE_PAYMENTACK = "application/zero-paymentack";
// const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/zero-paymentrequest";
//
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the zero: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(ZERO_IPC_PREFIX, Qt::CaseInsensitive)) // zero: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseZeroURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ZERO_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(ZERO_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click zero: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start zero: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling zero: URIs and PaymentRequest mime types.
...
//         delete netManager;
//
//     // netManager is used to fetch paymentrequests given in zero: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(ZERO_IPC_PREFIX, Qt::CaseInsensitive)) // zero: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseZeroURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Zero address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
//             // Unauthenticated payment requests to custom zero addresses are not supported
...
//         }
//
//         // Zero amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
...
//                 .arg(ZeroUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
//                 CClientUIInterface::MSG_ERROR);
//
//             return false;
//         }
