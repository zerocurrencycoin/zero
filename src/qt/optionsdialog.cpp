    ...
    mapper(0)
{
    ...
    ui->zeroAtStartup->setToolTip(ui->zeroAtStartup->toolTip().arg(tr(PACKAGE_NAME)));
    ui->zeroAtStartup->setText(ui->zeroAtStartup->text().arg(tr(PACKAGE_NAME)));

    ui->openZeroConfButton->setToolTip(ui->openZeroConfButton->toolTip().arg(tr(PACKAGE_NAME)));

    //Add Wallet themes available
    ui->theme->addItem("Zero", QVariant("zero"));
    ...
    ui->unit->setModel(new ZeroUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->zeroAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openZeroConfButton_clicked()
{
