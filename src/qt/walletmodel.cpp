...
extern uint64_t zero_interestsum();
    ...
    cachedNumBlocks(0)
{
    ...
    globalZeronodeTableModel = new GlobalZNTableModel(wallet, this);
    localZeronodeTableModel = new LocalZNTableModel(this);

    //read zeronode.conf into the table
    localZeronodeTableModel->refreshLocalZNData();
...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks)
    {
        ...
        if(addressTableModel)
            addressTableModel->updateBalances();

        if(zaddressTableModel)
            zaddressTableModel->updateBalances();

        if(globalZeronodeTableModel && localZeronodeTableModel)
            globalZeronodeTableModel->refreshGlobalZNData(localZeronodeTableModel);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        #endif
        {   // User-entered zero address / amount:
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                // if ( GetTime() > ZERO_SAPLING_DEADLINE )
                // {
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : transaction.getRecipients())
        {
            ...
            #endif
                if (!rcp.message.isEmpty()) // Message from normal zero:URI (zero:123...?message=example)
...
GlobalZNTableModel *WalletModel::getGlobalZNModel()
{
    return globalZeronodeTableModel;
}

LocalZNTableModel *WalletModel::getLocalZNModel()
{
    return localZeronodeTableModel;
}
