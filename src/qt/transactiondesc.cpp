...
QString TransactionDesc::toHTML(CWallet *wallet, TransactionRecord *rec, int unit)
{
    ...
    strHTML += "<b>" + tr("fee") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, -(arcTx.transparentValue + arcTx.sproutValue + arcTx.saplingValue)) + "<br>";
    ...
    if (arcTx.spentFrom.size() > 0) {
        for (int i = 0; i < arcTx.vTSend.size(); i++) {
            ...
            tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.vTSend[i].amount)) + "<br>";
            if (change) {
        ...
        for (int i = 0; i < arcTx.vZcReceived.size(); i++) {
            ...
            tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.vZcReceived[i].amount)) + "<br>";
            if (change) {
                tempHTML += "<b>" + tr("    Change") + ":</b> True<br>";
                sendChangeHTML += tempHTML;
            } else {
        ...
        if (arcTx.sproutValue - arcTx.sproutValueSpent - sproutValueReceived != 0) {
            ...
            tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.sproutValue - arcTx.sproutValueSpent)) + "<br>";
            tempHTML += "<b>" + tr("    Change") + ":</b> False<br>";
            sendHTML += tempHTML;
        }
        ...
        for (int i = 0; i < arcTx.vZsSend.size(); i++) {
            ...
            tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.vZsSend[i].amount)) + "<br>";
            if (change) {
                tempHTML += "<b>" + tr("    Change") + ":</b> True<br>";
            } else {
                allChange = false;
    ...
    for (int i = 0; i < arcTx.vTReceived.size(); i++) {
        ...
        tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.vTReceived[i].amount)) + "<br>";
        if (change) {
            tempHTML += "<b>" + tr("    Change") + ":</b> True<br>";
    ...
    for (int i = 0; i < arcTx.vZsReceived.size(); i++) {
        ...
        tempHTML += "<b>" + tr("    Value") + ":</b> " + ZeroUnits::formatHtmlWithUnit(unit, CAmount(arcTx.vZsReceived[i].amount)) + "<br>";
        if (change) {
            tempHTML += "<b>" + tr("    Change") + ":</b> True<br>";
        } else {
            allChange = false;
