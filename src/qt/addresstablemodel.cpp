...
struct AddressTableEntry
{
    ...
    AddressTableEntry() {}
    AddressTableEntry(Type _type, const QString &_label, const QString &_address, const CAmount &_balance):
...
extern CAmount getBalanceTaddr(std::string address, int minDepth, bool ignoreUnspendable);
...
public:
    ...
    void updateBalances() {
        ...
        for (int i = 0; i < cachedAddressTable.size(); ++i) {
            cachedAddressTable[i].balance = getBalanceTaddr(cachedAddressTable[i].address.toStdString(), 1, false);
        }
    }
...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DecorationRole || role == Qt::DisplayRole || role == Qt::EditRole)
    {
        if (index.column() == isMine)
        {
            {
                LOCK(wallet->cs_wallet);
                bool isValid = IsValidDestinationString(rec->address.toStdString());
...
void AddressTableModel::updateBalances() {
    priv->updateBalances();
}
...
{
    // Update address book model from Zero core
