...
UniValue getsaplingwitness(const UniValue& params, bool fHelp) {
  ...
  {
      if (!GetTransaction(ctxhash, ctx, Params().GetConsensus(), hashBlock, true))
  ...
  const bool saplingActive = Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_SAPLING);
  if (!saplingActive)
